...
function AttackNoCD()
    if not (_G.AutoFarmFruitMastery or _G.AutoFarmGunMastery) then
        if _G.FastAttack then
            ...
            local function GetEnemiesInRange(range)
                ...
                for _, enemy in ipairs(enemies) do
                    ...
                    if root and IsEntityAlive(enemy) and (root.Position - playerPos).Magnitude <= range then
            ...
            if equippedWeapon:FindFirstChild("LeftClickRemote") then
                ...
                for _, enemy in ipairs(enemiesInRange) do
                    ...
                    if attackCount > 1e9 then attackCount = 1 end
                end
            else
                local targets = {}
                ...
                for _, enemy in ipairs(enemies) do
                    if not enemy:GetAttribute("IsBoat") and IsEntityAlive(enemy) then
                        ...
                        if head and (head.Position - playerPos).Magnitude <= 60 then
            ...
            -- Nếu không bật FastAttack thì giả lập click chuột
...
spawn(function()
    while wait() do
        pcall(function()
            for i, v in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                if _G.BringMonster and _G.BringMode then
